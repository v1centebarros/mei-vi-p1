...
export const Stats = (props) => {
    ...
    return <div className={"grid grid-cols-8 m-2 gap-2"}>
        <Card title={"Scatter Plot"}>
            <ResponsiveWrapper>
                {({ width, height }) => <ScatterPlot data={data} width={width} height={height} margin={DEFAULT_MARGIN} />}
            </ResponsiveWrapper>
        </Card>

        <Card title={"Histogram"}>
            <ResponsiveWrapper>
                {({ width, height }) => <Histogram data={data} width={width} height={height} margin={DEFAULT_MARGIN} />}
            </ResponsiveWrapper>
        </Card>
        ...
        <Card title={"Force Pie Chart"}>
            <ResponsiveWrapper>
                {({ width, height }) => <ForcePieChart data={data} width={width} height={height} margin={DEFAULT_MARGIN}
                    selectedYear={2013} />}
            </ResponsiveWrapper>
        </Card>
        ...
        <Card title={"Gunfire Pie Chart"}>
            <ResponsiveWrapper>
                {({ width, height }) => <GunfirePieChart data={data} width={width} height={height} margin={DEFAULT_MARGIN}
                    selectedYear={2013} />}
            </ResponsiveWrapper>
        </Card>
